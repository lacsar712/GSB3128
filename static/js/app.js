...
async function showResults(eid) {
  ...
  openModal('考试成绩', `<div class="stat-cards" style="margin-bottom:16px">
    ...
    <div class="stat-card"><div class="num">${r.stats.pass_rate}%</div><div class="label">及格率(≥${passScore})</div></div>
  </div><div class="table-wrap"><table><tr><th>排名</th><th>姓名</th><th>分数</th><th>状态</th><th>交卷时间</th></tr>
  ${r.results.map(s => `<tr><td>${s.rank}</td><td>${s.name}</td><td><strong style="color:${s.score >= passScore ? 'var(--success)' : 'var(--danger)'}${''}">${s.score}</strong></td><td><span class="tag ${s.status === '已批阅' ? 'tag-success' : 'tag-warning'}">${s.status}</span></td><td>${s.submit_time}</td></tr>`).join('')}</table></div>`
